...
fn emit_map_helper(nodes: Vec<ASTNode>, delimiter: String) -> Result<String, Error> {
    let mut err = false;
    let name_vec: Vec<String> = nodes.iter().map(|node| {
    ...
    if err {
        return Err(Error { msg: String::from("Emit failed for a node in the list") })
    }
    ...
    -> Result<String, Error>
{
    ...
    Err(Error { msg: String::from("Malformed binary node") })
}
